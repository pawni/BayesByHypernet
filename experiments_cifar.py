...
import numpy as np
from utils_cifar import (x_train_first, y_train_first, build_result_dict,
                         get_probs, x_test_first, y_test_first)
import time
import os
...
def analysis(ops, session, save_path, num_samples, mod='bayesian'):
    ...
    print('In {} Out {}'.format(
        results['test_ent_auc'], results['outlier_ent_auc']))
    ...
    if not (mod == 'map' or mod == 'dropout' or mod == 'ensemble'):
        ...
        all_weights = all_weights[:, :3 * 3 * 16 * 3]
...
def run_klapprox_experiment(ops, config):
    ...
    batches_per_epoch = len(x_train_first) // batch_size
    ...
    with tf.Session() as s:
        ...
        with trange(epochs * batches_per_epoch) as pbar:  # run ~50 epochs
            for i in pbar:
                ...
                if i % batches_per_epoch == 0:
                    idx = 0
                    indices = rng.permutation(len(x_train_first))
                    # get batch from dataset
                batch_xs = x_train_first[indices[idx:idx + batch_size]]
                batch_ys = y_train_first[indices[idx:idx + batch_size]]
                ...
                if i % 1000 == 0:
                    if False:
                        probs = get_probs(
                            x_train_first[:1000], s, ops,
                            'bayesian', is_eval=True)
                        mean_probs = probs.mean(0)
                        val_acc = np.mean(
                            np.argmax(mean_probs, -1) == y_train_first[:1000])
        ...
        with open(os.path.join(save_path, "res.txt"), "a") as f:
            ...
            print('In {} out {}'.format(
                results['test_ent_auc'], results['outlier_ent_auc']), file=f)
...
def run_vanilla_experiment(ops, config):
    save_path = os.path.join(config['logdir'], config['experiment'])
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    seed = config['seed']
    ...
    with tf.Session() as s:
        ...
        with trange(epochs * batches_per_epoch) as pbar:  # run ~50 epochs
            for i in pbar:
                ...
                if i % 1000 == 0:
                    if True:
                        train_acc, probs = s.run(
                            [ops['acc'], ops['probs']],
                            {ops['x']: x_train_first[:1000],
                             ops['y']: y_train_first[:1000],
                             ops['is_eval']: True})
                        ...
                        train_acc_man = np.mean(
                            np.argmax(probs, -1) == y_train_first[:1000])
                        probs = get_probs(
                            x_train_first[:1000], s, ops,
                            'vanilla', is_eval=True)
                        mean_probs = probs.mean(0)
                        val_acc = np.mean(
                            np.argmax(mean_probs, -1) == y_train_first[:1000])

                        probs = get_probs(x_test_first, s, ops, 'vanilla',
                                          is_eval=True)
                        mean_probs = probs.mean(0)
                        test_acc = np.mean(
                            np.argmax(mean_probs, -1) == y_test_first)
                ...
                pbar.set_postfix(acc=fetched['acc'],
                                 ...
                                 test_acc=test_acc,
                                 train_acc=train_acc,
                                 train_acc_man=train_acc_man,
                                 train_acc_noe=train_acc_noe)
                idx += batch_size
    ...
    with tf.Session() as s:
        ...
        with trange(epochs * batches_per_epoch) as pbar:  # run ~50 epochs
            for i in pbar:
                ...
                feed_dict = {ops['x']: batch_xs, ops['y']: batch_ys,
                             anneal: annealing, ops['is_eval']: False}
        ...
        with trange(epochs * batches_per_epoch) as pbar:  # run ~50 epochs
            for i in pbar:
                if i % batches_per_epoch == 0:
                    idx = 0
                    indices = [rng.permutation(len(x_train_first))
                               for _ in range(len(ops['tot_loss']))]
                    # get batch from dataset

                # get batch from dataset
                ce = 0
                b_acc = 0
                for ens, (loss, acc, opt) in enumerate(
                        zip(ops['loss'], ops['acc'], optimiser)):
                    batch_xs = x_train_first[indices[ens][idx:idx + batch_size]]
                    batch_ys = y_train_first[indices[ens][idx:idx + batch_size]]
                    feed_dict = {ops['x']: batch_xs, ops['y']: batch_ys,
                                 ops['is_eval']: False}
