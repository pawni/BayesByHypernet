...
def get_bbh_cifar(ops, num_samples=5, sample_output=True, noise_shape=1,
                  layer_wise=False, slice_last_dim=False,
                  force_zero_mean=False,
                  num_slices=1, h_units=(256, 512)):
    ...
    if layer_wise:
        if slice_last_dim:
            num_slices = 192
        c1 = layers.BBHConvLayer('c1', 3, 192, 5, 'VALID',
                                 num_samples=num_samples, num_slices=num_slices,
                                 h_noise_shape=noise_shape,
                                 h_units=h_units, h_use_bias=h_use_bias)
        ...
        if slice_last_dim:
            ...
            num_slices = 24
        c2 = layers.BBHConvLayer('c2', 192, 192, 5, 'VALID',
                                 num_samples=num_samples, num_slices=num_slices,
                                 h_noise_shape=noise_shape,
                                 h_units=h_units, h_use_bias=h_use_bias)
        ...
        if slice_last_dim:
            ...
            num_slices = 25
        fc1 = layers.BBHDenseLayer('fc1', 1728, 1000, h_units=h_units,
                                   num_samples=num_samples,
                                   num_slices=num_slices,
                                   h_noise_shape=noise_shape,
                                   h_use_bias=h_use_bias)
        ...
        if slice_last_dim:
            num_slices = 5
        fc2 = layers.BBHDenseLayer('fc2', 1000, 5, h_units=h_units,
                                   num_samples=num_samples,
                                   num_slices=num_slices,
                                   h_noise_shape=noise_shape,
                                   h_use_bias=h_use_bias)
        ...
        with tf.variable_scope(base_layers.hypernet_vs):
            ...
            z = tf.layers.dense(z, 10639, use_bias=h_use_bias)
...
def get_bbb_cifar(ops, init_var=-15, prior_scale=1.):
    x = tf.placeholder(tf.float32, [None, 32, 32, 3])
    y = tf.placeholder(tf.int32, [None])
    adv_eps = tf.placeholder_with_default(1e-2, [])
    ...
    c1 = layers.BBBConvLayer('c1', 3, 192, 5, 'VALID', init_var=init_var,
                             prior_scale=prior_scale)

    c2 = layers.BBBConvLayer('c2', 192, 192, 5, 'VALID', init_var=init_var,
                             prior_scale=prior_scale)

    fc1 = layers.BBBDenseLayer('fc1', 1728, 1000, init_var=init_var,
                               prior_scale=prior_scale)

    fc2 = layers.BBBDenseLayer('fc2', 1000, 5, init_var=init_var,
                               prior_scale=prior_scale)
...
def get_vanilla_cifar(ops):
    ...
    c1 = layers.VanillaConvLayer('c1', 3, 192, 5, 'VALID')

    c2 = layers.VanillaConvLayer('c2', 192, 192, 5, 'VALID')

    fc1 = layers.VanillaDenseLayer('fc1', 1728, 1000)

    fc2 = layers.VanillaDenseLayer('fc2', 1000, 5)
...
def get_bbh_net(ops, num_samples=5, sample_output=True, num_slices=1,
                h_units=(256, 512), noise_shape=1):
    x = tf.placeholder(tf.float32, [None, 784])
    y = tf.placeholder(tf.int32, [None])
    adv_eps = tf.placeholder_with_default(1e-2, [])
    ...
    fc1 = layers.BBHDenseLayer('fc1', 1728, 500, h_units=h_units,
                               num_samples=num_samples, num_slices=num_slices,
                               h_noise_shape=noise_shape,
                               h_use_bias=h_use_bias)
...
#             for i in range(10):
#                 with tf.variable_scope('ens{}'.format(i)):
#                     conv1 = tf.layers.Conv2D(kernel_size=5, filters=20,
#                                              activation=tf.nn.relu,
#                                              padding='VALID',
#                                              kernel_initializer=tf.variance_scaling_initializer())
#                     conv2 = tf.layers.Conv2D(kernel_size=5, filters=50,
#                                              activation=tf.nn.relu,
#                                              padding='VALID',
#                                              kernel_initializer=tf.variance_scaling_initializer())
#                     fc1 = tf.layers.Dense(units=500, activation=tf.nn.relu,
#                                           kernel_initializer=tf.variance_scaling_initializer())
#                     fc2 = tf.layers.Dense(units=10,
#                                           kernel_initializer=tf.variance_scaling_initializer())
#
#                     def get_out(h):
#                         h = conv1(h)
...
#
#                 # conv 1
#                 c1 = layers.BBBConvLayer('c1', 1, 20, 5, 'VALID',
#                                          init_var=init_var)
#
#                 x = c1(x)
#                 x = tf.nn.relu(x)
...
#
#                 # conv 2
#                 c2 = layers.BBBConvLayer('c2', 20, 50, 5, 'VALID',
#                                          init_var=init_var)
#
#                 x = c2(x)
#                 x = tf.nn.relu(x)
...
#
#                 # conv 1
#                 c1 = layers.MNFConvLayer('c1', 1, 20, 5, 'VALID', thres_var=0.5,
#                                          learn_p=learn_p)
#
#                 x = c1(x)
#                 x = tf.nn.relu(x)
...
#
#                 # fc 1
#                 fc1 = layers.MNFDenseLayer('fc1', 800, 500, thres_var=0.5,
#                                            learn_p=learn_p)
#
#                 x = fc1(x)
#                 x = tf.nn.relu(x)
#
#                 # fc 2
#                 fc2 = layers.MNFDenseLayer('fc2', 500, 10, thres_var=0.5,
#                                            learn_p=learn_p)
#
#                 x = fc2(x)
#
#                 kl_loss = tf.add_n(tf.get_collection('mnf_kl'))
#
#                 ops['kl_loss'] = kl_loss
#
#             elif mode == 'dropout':  # dropout
#                 d_pi = 0.5
#                 x = tf.layers.conv2d(inputs=x, kernel_size=5, filters=20,
#                                      activation=tf.nn.relu, padding='VALID',
#                                      kernel_initializer=tf.variance_scaling_initializer())
#                 x = tf.nn.dropout(x, d_pi)
#
#                 x = tf.nn.max_pool(x, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
#                                    padding='SAME')
#
...
#                 x = tf.layers.flatten(x)
#
#                 x = tf.layers.dense(inputs=x, units=500,
#                                     activation=tf.nn.relu,
#                                     kernel_initializer=tf.variance_scaling_initializer())
#                 x = tf.nn.dropout(x, d_pi)
#
